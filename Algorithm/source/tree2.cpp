/**
 * Assignment 06 : Tree Construction II
 */
...
int inOrder_root(int root, int left, int right) {
    ...
    if (orderCheck[0]) {
        while (in[left] != pre[root]) left++;
        return left;
    } else {
        while (in[left] != post[root]) left++;
        return left;
    }
}
...
void preOrder(int root, int left, int right) {
    ...
    if (left < right) {
        if (L_size != 0) preOrder(L, left, in_root - 1);
        if (R_size != 0) preOrder(R, in_root + 1, right);
    }
}
...
int main() {
    ...
    if (orderCheck[0]) {
        ...
        for (int i = 0; i < post.size(); i++) out << post[i] << "\n";
    } else {
        preOrder(n - 1, 0, n - 1);  // 후위순회일  root의 인덱스는
                                    // 노드의 개수 -1
